            ...
            # CARGA DESDE LA RED (Escenario 4)
                ...
                if self.descarga_red:
                    ...
                    if precio_actual_kWh < umbral_rentabilidad_kWh:
                        es_rentable_descargar = False # No cubre costes mínimos
                    else:
                        # 3. Buscamos si quedan horas HOY que sean estrictamente más caras que la actual y tengan demanda
                        horas_futuras_mas_caras = [
                            h for h in range(1, len(precios_restantes))
                            if (precios_restantes[h] / 1000) > precio_actual_kWh and demanda_restante[h] > 0
                        ]
                        
                        if not horas_futuras_mas_caras:
                            # Esta hora actual es la mejor oportunidad de precio que queda en el día. ¡Descargamos!
                            es_rentable_descargar = True
                        else:
                            # 4. Hay horas más caras más tarde. Calculamos cuánta energía necesitamos reservar para ellas
                            energia_reservada_futura_AC = 0.0
                            for h_futura in horas_futuras_mas_caras:
                                energia_reservada_futura_AC += min(demanda_restante[h_futura], self.potencia_inv)
                            
                            energia_disponible_quimica = max(0, soc_actual - self.soc_min_kwh)
                            energia_disponible_AC = energia_disponible_quimica * self.rend_descarga
                            
                            # Si tenemos más energía que la que van a necesitar los picos más altos del futuro, 
                            # descargamos el excedente ahora mismo para evitar que se quede estancada.
                            if energia_disponible_AC > energia_reservada_futura_AC:
                                es_rentable_descargar = True
                            else:
                                es_rentable_descargar = False # Guardamos la energía para más tarde

                # --- EJECUCIÓN FISICA DE LA DESCARGA (CORRECCIÓN DE INDENTACIÓN) ---
                # Fíjate que este "if" ahora está alineado verticalmente con el "if self.descarga_red:" de arriba.
