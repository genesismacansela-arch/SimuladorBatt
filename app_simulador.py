...
if tiene_bateria:
    ...
    cd_rate = st.sidebar.number_input("C-rate Descarga", min_value=0.0, value=1.0, max_value=1.0, step=0.5)
    cc_rate = st.sidebar.number_input("C-rate Carga", min_value=0.0, value=1.0, max_value=1.0, step=0.5)
...
def ejecutar_motor_optimizado(df_inputs, permitir_inyeccion, tiene_bateria, cap_bateria, pot_inversor, soc_maximo, soc_minimo,
                              soc_inicial, rend_carga, rend_descarga, potencia_fv_kwp, perdidas_fv,max_ciclos, cd_rate, cc_rate,
                              descarga_red, precio_min_descarga, permitir_carga_red, precio_max_carga, coste_ciclo, deg_ciclo):
    ...
    simulador = BESS_Simulator(
        ...
        cd_rate=cd_rate if tiene_bateria else 1.0,
        cc_rate=cc_rate if tiene_bateria else 1.0,
...
#--------------------------------------------------------------------------------------------------
        ...
        tasa_autoconsumo = (autoconsumo_total / demanda_anual * 100) if demanda_anual > 0 else 0
        dependencia = (Consumo_Red / demanda_anual * 100) if demanda_anual > 0 else 0
        soberania = max(0, 100 - dependencia)
        
        col1, col2, col3 = st.columns(3)

        nombre_kpi_principal = "Tasa de Autoconsumo" if permitir_carga_red else "Penetración Renovable"
        if tiene_bateria and autoconsumo_total > 0:
            ...
            if permitir_carga_red:
                delta_autoconsumo = f"{formato_es(aporte_FV, 0)}% directo FV | {formato_es(aporte_bat, 0)}% aporte batería mixta"
        elif autoconsumo_total > 0:
            delta_autoconsumo = ""
        else:
            delta_autoconsumo = "No se detecto autoconsumo" 
        col1.metric(
            label=nombre_kpi_principal, 
            value=f"{tasa_autoconsumo:.1f}%",
        ...
        if permitir_carga_red and 'CARGA BAT. RED (kWh)' in df_final.columns and Consumo_Red > 0:
            ...
            delta_dependencia = f"{formato_es(pct_red_bat, 1)}% a batería | {formato_es(soberania, 1)}% de soberania energetica"
        else:
            delta_dependencia = ""   
        col2.metric(
            label="Dependencia de Red", 
            value=f"{formato_es(dependencia, 1)}%", 
        ...
        if tiene_bateria and autoconsumo_total > 0:    
            delta_text = f"{formato_es(porcentaje_gestion, 1)}% del sobrante"
        else:
            delta_text =""
        col3.metric(
            label=f"Excedente de Producción", 
            value=f"{formato_es(porcentaje_total, 1)}%", 
            delta=delta_text
        )
        ...
        with st.expander(" Métricas Globales "):
            ...
            c2.metric("Demanda", f"{formato_es(df_final['CONSUMO HORA (kWh)'].sum())} kWh")
            if 'PRECIO RED (€/MWh)' in df_final.columns:
                gasto_total_eur = (df_final['CONSUMO DE RED (kWh)'] * df_final['PRECIO RED (€/MWh)'] / 1000).sum()
                delta_text = f"Gasto Total Red: {formato_es(gasto_total_eur, 2)} €"
            else:
                delta_text = "Gasto Total Red: Sin datos de precio"
            ...
            c4.metric("Autoconsumo Total", f"{formato_es(df_final['AUTOCONSUMO TOTAL (kWh)'].sum())} kWh")
            c5.metric("Sobrante FV", f"{formato_es(df_final['SOBRANTE (kWh)'].sum())} kWh")
            c6.metric(
        ...
        if tiene_bateria and cap_bateria > 0:
            with st.expander(" Análisis de Batería"):
                ...
                ventana_soc = (soc_maximo - soc_minimo) * 100
                ...
                c_util.metric(
                    label="Capacidad Útil Real", 
                    value=f"{formato_es(capacidad_util, 0)} kWh", 
                    delta=f"Ventana operativa: {formato_es(ventana_soc, 0)}%", 
                    delta_color="normal"
                )
...
# --------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        if tiene_bateria:
            parametros.extend([
                "Capacidad Batería (kWh)", "C-Rate Carga", "C-Rate Descarga", "Max Ciclos Diarios", "SOC Mínimo (%)", 
            ...
            valores.extend([
                cap_bateria, cc_rate, cd_rate, max_ciclos, soc_minimo * 100, soc_maximo * 100, soc_inicial * 100,
...
# ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        #  GENERACIÓN CONDICIONAL Y DINÁMICA DE ESCENARIOS 
        
        
        if tiene_bateria:
            # Siempre generamos E1 y E2 como base comparativa si hay batería
            df_e1, _ = ejecutar_motor_optimizado(
                df_inputs=df_inputs, permitir_inyeccion=permitir_inyeccion, tiene_bateria=False,
                cap_bateria=0.0, pot_inversor=pot_inversor, soc_maximo=1.0, soc_minimo=0.0, soc_inicial=0.0, 
                rend_carga=1.0, rend_descarga=1.0, potencia_fv_kwp=potencia_fv_kwp, perdidas_fv=perdidas_fv, 
                max_ciclos=1, cd_rate=1.0, cc_rate=1.0, descarga_red=False, precio_min_descarga=0.0, permitir_carga_red=False, precio_max_carga=0.0, coste_ciclo=0.0, deg_ciclo=0.0
            )
            
            df_e2, kpis_e2 = ejecutar_motor_optimizado(
                df_inputs=df_inputs, permitir_inyeccion=permitir_inyeccion, tiene_bateria=True,
                cap_bateria=cap_bateria, pot_inversor=pot_inversor, soc_maximo=soc_maximo, soc_minimo=soc_minimo, soc_inicial=soc_inicial, 
                rend_carga=rend_carga, rend_descarga=rend_descarga, potencia_fv_kwp=potencia_fv_kwp, perdidas_fv=perdidas_fv, 
                max_ciclos=max_ciclos, cc_rate=cc_rate, cd_rate=cd_rate, descarga_red=False, precio_min_descarga=0.0, permitir_carga_red=False, precio_max_carga=0.0, coste_ciclo=0.0, deg_ciclo=deg_ciclo
            )
            
            # E3: Solo si activas "Programar descargas"
            if descarga_red:
                df_e3, kpis_e3 = ejecutar_motor_optimizado(
                    df_inputs=df_inputs, permitir_inyeccion=permitir_inyeccion, tiene_bateria=True,
                    cap_bateria=cap_bateria, pot_inversor=pot_inversor, soc_maximo=soc_maximo, soc_minimo=soc_minimo, soc_inicial=soc_inicial, 
                    rend_carga=rend_carga, rend_descarga=rend_descarga, potencia_fv_kwp=potencia_fv_kwp, perdidas_fv=perdidas_fv, 
                    max_ciclos=max_ciclos, cc_rate=cc_rate, cd_rate=cd_rate, descarga_red=True, precio_min_descarga=precio_min_descarga, permitir_carga_red=False, precio_max_carga=0.0, coste_ciclo=coste_ciclo, deg_ciclo=deg_ciclo
                )
            
            # E4: Solo si activas "Cargar desde red"
            if permitir_carga_red:
                df_e4, kpis_e4 = ejecutar_motor_optimizado(
                    df_inputs=df_inputs, permitir_inyeccion=permitir_inyeccion, tiene_bateria=True,
                    cap_bateria=cap_bateria, pot_inversor=pot_inversor, soc_maximo=soc_maximo, soc_minimo=soc_minimo, soc_inicial=soc_inicial, 
                    rend_carga=rend_carga, rend_descarga=rend_descarga, potencia_fv_kwp=potencia_fv_kwp, perdidas_fv=perdidas_fv, 
                    max_ciclos=max_ciclos, cc_rate=cc_rate, cd_rate=cd_rate, descarga_red=False, precio_min_descarga=0.0, permitir_carga_red=True, precio_max_carga=precio_max_carga, coste_ciclo=0.0, deg_ciclo=deg_ciclo
                )
            
            # E5: Solo si AMBAS están activas (Híbrido)
            if descarga_red and permitir_carga_red:
                df_e5, kpis_e5 = ejecutar_motor_optimizado(
                    df_inputs=df_inputs, permitir_inyeccion=permitir_inyeccion, tiene_bateria=True,
                    cap_bateria=cap_bateria, pot_inversor=pot_inversor, soc_maximo=soc_maximo, soc_minimo=soc_minimo, soc_inicial=soc_inicial, 
                    rend_carga=rend_carga, rend_descarga=rend_descarga, potencia_fv_kwp=potencia_fv_kwp, perdidas_fv=perdidas_fv, 
                    max_ciclos=max_ciclos, cc_rate=cc_rate, cd_rate=cd_rate, descarga_red=True, precio_min_descarga=precio_min_descarga, permitir_carga_red=True, precio_max_carga=precio_max_carga, coste_ciclo=coste_ciclo, deg_ciclo=deg_ciclo
                )
        else:
            df_e1 = df_final
        ...
        def agrupar_a_mensual(df_horario):
            ...
            # --- CORRECCIÓN: Variable dinámica para el nombre de la columna ---
            nombre_kpi = 'Tasa Autoconsumo (%)' if permitir_carga_red else 'Penetración de Energía Renovable (%)'
            
            # Aplicamos la matemática usando el nombre correcto
            df_mes[nombre_kpi] = (df_mes['Autoconsumo (kWh)'] / df_mes['Demanda (kWh)']) * 100
            ...
            # Calculamos totales asegurando que solo suma columnas numéricas
            totales = df_mes.sum(numeric_only=True)
            totales[nombre_kpi] = (totales['Autoconsumo (kWh)'] / totales['Demanda (kWh)']) * 100
            totales.name = 'TOTAL'
            
            return pd.concat([df_mes, pd.DataFrame(totales).T])

        # Generación Condicional de df_mensual
        df_mensual_e1 = agrupar_a_mensual(df_e1)
        if tiene_bateria:
            df_mensual_e2 = agrupar_a_mensual(df_e2) 
            if descarga_red: df_mensual_e3 = agrupar_a_mensual(df_e3)
            if permitir_carga_red: df_mensual_e4 = agrupar_a_mensual(df_e4)
            if descarga_red and permitir_carga_red: df_mensual_e5 = agrupar_a_mensual(df_e5)

        # --- FUNCIÓN DE RENDERIZADO DE GRÁFICOS PARA EXCEL ---
        def generar_figura_exportacion(df_plot, titulo):
            ...
            fig.update_layout(
                ...
                yaxis_title="Energía (kWh)", xaxis_title="Mes",
                legend=dict(orientation="h", yanchor="top", y=-0.25, xanchor="center", x=0.5), 
                margin=dict(l=80, r=40, t=60, b=120), height=500, width=900
            )
            return fig

        # ======================================================================================================================
        # --- CONSTRUCCIÓN DE LA TABLA MATRICIAL CONDICIONAL ---
        # ======================================================================================================================
        
        totales_e1 = df_mensual_e1.loc['TOTAL']
        dict_comparativa = {"Métrica": totales_e1.index, "E1 - Sin Batería": totales_e1.values}
        
        if tiene_bateria:
            dict_comparativa["E2 - Con Batería"] = df_mensual_e2.loc['TOTAL'].values
            if descarga_red: dict_comparativa["E3 - Descarga Red"] = df_mensual_e3.loc['TOTAL'].values
            if permitir_carga_red: dict_comparativa["E4 - Carga Red"] = df_mensual_e4.loc['TOTAL'].values
            if descarga_red and permitir_carga_red: dict_comparativa["E5 - Híbrido C/D Red"] = df_mensual_e5.loc['TOTAL'].values
            
        df_comparativa = pd.DataFrame(dict_comparativa)

        if tiene_bateria:
            # Función auxiliar para crear filas de forma dinámica según lo seleccionado
            def crear_fila_kpi(metrica, v_e1, v_e2, df_o_kpi_e3=None, df_o_kpi_e4=None, df_o_kpi_e5=None, tipo="kpi"):
                fila = {"Métrica": metrica, "E1 - Sin Batería": v_e1, "E2 - Con Batería": v_e2}
                if descarga_red:
                    fila["E3 - Descarga Red"] = df_o_kpi_e3.get(metrica.split().lower(), 0.0) if tipo == "kpi" else df_o_kpi_e3
                if permitir_carga_red:
                    fila["E4 - Carga Red"] = df_o_kpi_e4.get(metrica.split().lower(), 0.0) if tipo == "kpi" else df_o_kpi_e4
                if descarga_red and permitir_carga_red:
                    fila["E5 - Híbrido C/D Red"] = df_o_kpi_e5.get(metrica.split().lower(), 0.0) if tipo == "kpi" else df_o_kpi_e5
                return fila

            # Extraemos valores para evitar código repetitivo
            c_e2 = round(kpis_e2.get("ciclos_equivalentes", 0.0), 1)
            c_e3 = round(kpis_e3.get("ciclos_equivalentes", 0.0), 1) if descarga_red else 0
            c_e4 = round(kpis_e4.get("ciclos_equivalentes", 0.0), 1) if permitir_carga_red else 0
            c_e5 = round(kpis_e5.get("ciclos_equivalentes", 0.0), 1) if (descarga_red and permitir_carga_red) else 0

            d_e2 = round(kpis_e2.get("degradacion_anual", 0.0), 3)
            d_e3 = round(kpis_e3.get("degradacion_anual", 0.0), 3) if descarga_red else 0
            d_e4 = round(kpis_e4.get("degradacion_anual", 0.0), 3) if permitir_carga_red else 0
            d_e5 = round(kpis_e5.get("degradacion_anual", 0.0), 3) if (descarga_red and permitir_carga_red) else 0

            max_c_e2 = df_e2['CARGA BATERIA (kWh)'].max() if 'CARGA BATERIA (kWh)' in df_e2.columns else 0.0
            max_c_e3 = df_e3['CARGA BATERIA (kWh)'].max() if (descarga_red and 'CARGA BATERIA (kWh)' in df_e3.columns) else 0.0
            max_c_e4 = df_e4['CARGA BATERIA (kWh)'].max() if (permitir_carga_red and 'CARGA BATERIA (kWh)' in df_e4.columns) else 0.0
            max_c_e5 = df_e5['CARGA BATERIA (kWh)'].max() if (descarga_red and permitir_carga_red and 'CARGA BATERIA (kWh)' in df_e5.columns) else 0.0

            max_d_e2 = df_e2['DESCARGA DE BATERIA (kWh)'].max() if 'DESCARGA DE BATERIA (kWh)' in df_e2.columns else 0.0
            max_d_e3 = df_e3['DESCARGA DE BATERIA (kWh)'].max() if (descarga_red and 'DESCARGA DE BATERIA (kWh)' in df_e3.columns) else 0.0
            max_d_e4 = df_e4['DESCARGA DE BATERIA (kWh)'].max() if (permitir_carga_red and 'DESCARGA DE BATERIA (kWh)' in df_e4.columns) else 0.0
            max_d_e5 = df_e5['DESCARGA DE BATERIA (kWh)'].max() if (descarga_red and permitir_carga_red and 'DESCARGA DE BATERIA (kWh)' in df_e5.columns) else 0.0

            tot_d_e2 = round(df_e2['DESCARGA DE BATERIA (kWh)'].sum(), 2) if 'DESCARGA DE BATERIA (kWh)' in df_e2.columns else 0.0
            tot_d_e3 = round(df_e3['DESCARGA DE BATERIA (kWh)'].sum(), 2) if (descarga_red and 'DESCARGA DE BATERIA (kWh)' in df_e3.columns) else 0.0
            tot_d_e4 = round(df_e4['DESCARGA DE BATERIA (kWh)'].sum(), 2) if (permitir_carga_red and 'DESCARGA DE BATERIA (kWh)' in df_e4.columns) else 0.0
            tot_d_e5 = round(df_e5['DESCARGA DE BATERIA (kWh)'].sum(), 2) if (descarga_red and permitir_carga_red and 'DESCARGA DE BATERIA (kWh)' in df_e5.columns) else 0.0

            cost_e1 = round((df_e1['CONSUMO DE RED (kWh)'] * df_e1['PRECIO RED (€/MWh)'] / 1000).sum(), 2) if 'PRECIO RED (€/MWh)' in df_e1.columns else 0.0
            cost_e2 = round((df_e2['CONSUMO DE RED (kWh)'] * df_e2['PRECIO RED (€/MWh)'] / 1000).sum(), 2) if 'PRECIO RED (€/MWh)' in df_e2.columns else 0.0
            cost_e3 = round((df_e3['CONSUMO DE RED (kWh)'] * df_e3['PRECIO RED (€/MWh)'] / 1000).sum(), 2) if (descarga_red and 'PRECIO RED (€/MWh)' in df_e3.columns) else 0.0
            cost_e4 = round((df_e4['CONSUMO DE RED (kWh)'] * df_e4['PRECIO RED (€/MWh)'] / 1000).sum(), 2) if (permitir_carga_red and 'PRECIO RED (€/MWh)' in df_e4.columns) else 0.0
            cost_e5 = round((df_e5['CONSUMO DE RED (kWh)'] * df_e5['PRECIO RED (€/MWh)'] / 1000).sum(), 2) if (descarga_red and permitir_carga_red and 'PRECIO RED (€/MWh)' in df_e5.columns) else 0.0

            filas_bateria = [
                crear_fila_kpi("Ciclos Reales Equivalentes Bateria (ciclos/año)", 0.0, c_e2, c_e3, c_e4, c_e5, "val"),
                crear_fila_kpi("Degradación Anual Calculada Bateria (% anual)", 0.0, d_e2, d_e3, d_e4, d_e5, "val"),
                crear_fila_kpi("Máxima Carga Batería (kWh)", 0.0, max_c_e2, max_c_e3, max_c_e4, max_c_e5, "val"),
                crear_fila_kpi("Máxima Descarga Batería (kWh)", 0.0, max_d_e2, max_d_e3, max_d_e4, max_d_e5, "val"),
                crear_fila_kpi("Energía Total Aportada por Batería (kWh)", 0.0, tot_d_e2, tot_d_e3, tot_d_e4, tot_d_e5, "val"),
                crear_fila_kpi("COSTO TOTAL IMPORTACIÓN RED (€)", cost_e1, cost_e2, cost_e3, cost_e4, cost_e5, "val")
            ]
            df_comparativa = pd.concat([df_comparativa, pd.DataFrame(filas_bateria)], ignore_index=True)

        # ======================================================================================================================
        # --- EXPORTACIÓN CON DETECCIÓN DE CAMBIOS EN TIEMPO REAL (SESSION STATE) ---
        # ======================================================================================================================
        
        st.divider()
        st.markdown("### Exportación de Resultados")
        
        # 1. Creamos la "huella dactilar" con el estado exacto de todos los inputs actuales
        huella_parametros_actuales = (
            potencia_fv_kwp, perdidas_fv, pot_inversor, tiene_bateria, permitir_inyeccion,
            cap_bateria if tiene_bateria else 0.0,
            cd_rate if tiene_bateria else 1.0, 
            cc_rate if tiene_bateria else 1.0, 
            max_ciclos if tiene_bateria else 1.0,
            soc_minimo if tiene_bateria else 0.0,
            soc_maximo if tiene_bateria else 1.0,
            soc_inicial if tiene_bateria else 0.0,
            rend_carga if tiene_bateria else 1.0,
            rend_descarga if tiene_bateria else 1.0,
            descarga_red if tiene_bateria else False,
            precio_min_descarga if tiene_bateria else 0.0,
            permitir_carga_red if tiene_bateria else False,
            precio_max_carga if tiene_bateria else 0.0,
            coste_ciclo if tiene_bateria else 0.0,
            deg_ciclo if tiene_bateria else 0.0
        )

        # Función interna normal para la escritura pesada
        def fabricar_excels_pesados():
            import io
            import os
            import tempfile
            ...
            out_horario = io.BytesIO()
            with pd.ExcelWriter(out_horario, engine='openpyxl') as writer:
                ...
                if tiene_bateria:
                    ...
                    if descarga_red: df_e3.to_excel(writer, index=False, sheet_name="E3 - Descarga Red")
                    if permitir_carga_red: df_e4.to_excel(writer, index=False, sheet_name="E4 - Carga Red")
                    if descarga_red and permitir_carga_red: df_e5.to_excel(writer, index=False, sheet_name="E5 - Híbrido CD")
            
            out_mensual = io.BytesIO()
            with tempfile.TemporaryDirectory() as tmpdir:
                with pd.ExcelWriter(out_mensual, engine='openpyxl') as writer:
                    df_config.to_excel(writer, index=False, sheet_name="Configuración", float_format="%.2f")
                    df_comparativa.to_excel(writer, index=False, sheet_name="Comparativa Matrix", startrow=1, startcol=1, float_format="%.2f")
                    
                    try:
                        hoja_graficos = writer.sheets["Comparativa Matrix"]
                        fila_actual = len(df_comparativa) + 5
                        ...
                        hoja_graficos.add_image(XLImage(ruta_e1), f'B{fila_actual}')
                        
                        if tiene_bateria:
                            fig_e2 = generar_figura_exportacion(df_mensual_e2.drop('TOTAL', errors='ignore'), "Escenario 2: Con Batería Base")
                            ...
                            fila_sig_1 = fila_actual + 27
                            if descarga_red:
                                fig_e3 = generar_figura_exportacion(df_mensual_e3.drop('TOTAL', errors='ignore'), "Escenario 3: Descarga Inteligente")
                                ruta_e3 = os.path.join(tmpdir, "e3.png")
                                fig_e3.write_image(ruta_e3, format="png", scale=1)
                                hoja_graficos.add_image(XLImage(ruta_e3), f'B{fila_sig_1}')
                                
                            if permitir_carga_red:
                                fig_e4 = generar_figura_exportacion(df_mensual_e4.drop('TOTAL', errors='ignore'), "Escenario 4: Carga Inteligente")
                                ruta_e4 = os.path.join(tmpdir, "e4.png")
                                fig_e4.write_image(ruta_e4, format="png", scale=1)
                                hoja_graficos.add_image(XLImage(ruta_e4), f'F{fila_sig_1}')
                            
                            if descarga_red and permitir_carga_red:
                                fila_sig_2 = fila_sig_1 + 27
                                fig_e5 = generar_figura_exportacion(df_mensual_e5.drop('TOTAL', errors='ignore'), "Escenario 5: Arbitraje Híbrido Completo")
                                ruta_e5 = os.path.join(tmpdir, "e5.png")
                                fig_e5.write_image(ruta_e5, format="png", scale=1)
                                hoja_graficos.add_image(XLImage(ruta_e5), f'B{fila_sig_2}')
                    except Exception:
                        pass 
                    ...
                    if tiene_bateria:
                        ...
                        if descarga_red: df_mensual_e3.round(2).to_excel(writer, sheet_name="E3 - Descarga Red", float_format="%.2f")
                        if permitir_carga_red: df_mensual_e4.round(2).to_excel(writer, sheet_name="E4 - Carga Red", float_format="%.2f")
                        if descarga_red and permitir_carga_red: df_mensual_e5.round(2).to_excel(writer, sheet_name="E5 - Híbrido CD", float_format="%.2f")
            ...
            return out_horario.getvalue(), out_mensual.getvalue()

        # 2. SISTEMA DE ALERTA DE CAMBIOS
        reportes_desactualizados = False
        if 'huella_guardada' in st.session_state:
            # Si las variables actuales NO coinciden con las de la última generación...
            if st.session_state['huella_guardada'] != huella_parametros_actuales:
                reportes_desactualizados = True
                st.warning("⚠️ **Cambios detectados en la simulación:** Se han modificado parámetros en la barra lateral de control. Generar los reportes nuevamente para actualizar los archivos Excel.")

        # BOTÓN PREPARADOR
        if st.button(" Generar Reportes Excel", use_container_width=True):
            with st.spinner("Renderizando los escenarios y gráficas....."):
                bytes_horario, bytes_mensual = fabricar_excels_pesados()
                # Guardamos los archivos de esta corrida
                st.session_state['bytes_horario'] = bytes_horario
                st.session_state['bytes_mensual'] = bytes_mensual
                # Guardamos la huella digital de los parámetros de ESTA corrida
                st.session_state['huella_guardada'] = huella_parametros_actuales
            st.success("¡Archivos empaquetados y listos para descargar!")
            st.rerun() # Fuerza un refresco limpio para borrar el aviso ámbar al instante

        # BOTONES DE DESCARGA (Solo aparecen si existen los archivos en memoria)
        if 'bytes_mensual' in st.session_state and 'bytes_horario' in st.session_state:
            col_btn1, col_btn2 = st.columns(2)
            with col_btn1:
                st.download_button(
                    label=" Descargar Resumen Mensual" if not reportes_desactualizados else " Descargar Resumen Mensual (Desactualizado)",
                    data=st.session_state['bytes_mensual'],
                    file_name="Balance_Mensual_Escenarios.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    type="primary" if not reportes_desactualizados else "secondary",
                    use_container_width=True
                )
            with col_btn2:
                st.download_button(
                    label=" Descargar Balance Horario Completo" if not reportes_desactualizados else " Descargar Balance Horario (Desactualizado)",
                    data=st.session_state['bytes_horario'],
